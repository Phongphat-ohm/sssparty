...
export default async function StudentAssignmentDetailPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Link
            ...
            className="p-2.5 rounded-2xl bg-white border border-[#EADBCC] text-[#7A6A5C] hover:text-[#3F342B] hover:border-[#D9A441] transition-all shadow-2xs"
        ...
        <div className="lg:hidden flex items-center justify-end">
          <a
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#D9A441] text-white text-xs font-bold shadow-xs hover:bg-[#C28F30] transition-colors"
          >
      ...
      {/* 2. TWO-COLUMN CLASSROOM LAYOUT */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* LEFT COLUMN: Instructions, Attachments, Rubrics (col-span-7 / 8) */}
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          {/* Assignment Description Card */}
          <div className="bg-white rounded-3xl p-5 sm:p-7 border border-[#EADBCC] shadow-xs space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#F2E8DC] pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-[#7A6A5C] flex items-center gap-1 font-medium">
            ...
            <div className="space-y-2">
              <h2 className="text-xs font-bold uppercase tracking-wider text-[#7A6A5C]">
                คำสั่งและรายละเอียดภาระงาน
              </h2>
              <div className="bg-[#FAF6F0] p-4 sm:p-5 rounded-2xl border border-[#EADBCC]">
            ...
            {assignment.attachments.length > 0 && (
              <div className="space-y-3 pt-3 border-t border-[#F2E8DC]">
                <div className="flex items-center gap-1.5">
                  <Paperclip className="w-4 h-4 text-[#D9A441]" />
                  <h3 className="text-xs font-bold text-[#3F342B]">
                    เอกสารประกอบโจทย์จากครู ({assignment.attachments.length} รายการ)
                  </h3>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {assignment.attachments.map((att) => {
                    ...
                    return (
                      <div
                        key={att.id}
                        className="p-3 rounded-2xl bg-[#FAF6F0] border border-[#EADBCC] flex flex-col justify-between gap-2.5 shadow-2xs hover:border-[#D9A441] transition-all"
                      >
                        {isImg && (
                          <div className="w-full h-36 rounded-xl overflow-hidden bg-black/5 border border-[#EADBCC]">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 overflow-hidden min-w-0">
                            <div className="w-8 h-8 rounded-xl bg-white border border-[#EADBCC] flex items-center justify-center shrink-0">
          ...
          {assignment.rubrics.length > 0 && (
            <div className="bg-white rounded-3xl p-5 sm:p-7 border border-[#EADBCC] shadow-xs space-y-3.5">
              <div className="flex items-center justify-between border-b border-[#F2E8DC] pb-3">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-xl bg-[#D9A441]/15 text-[#D9A441] flex items-center justify-center">
                    <FileCheck className="w-4 h-4" />
                  </div>
                  <div>
                    <h2 className="text-sm sm:text-base font-bold text-[#3F342B]">
                      เกณฑ์การให้คะแนน (Rubrics)
                    </h2>
                    <p className="text-[11px] text-[#7A6A5C]">
                      เกณฑ์การประเมินมาตรฐาน {assignment.rubrics.length} ข้อ
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-2.5">
                {assignment.rubrics.map((rubric, idx) => {
                  ...
                  return (
                    <div
                      key={rubric.id}
                      className="p-3.5 bg-[#FAF6F0] rounded-2xl border border-[#EADBCC] flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                    >
                      <div className="space-y-1 max-w-xl">
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-bold text-[#8C5D23] bg-white px-2 py-0.5 rounded-md border border-[#EADBCC]">
                            ข้อที่ {idx + 1}
                          </span>
                      ...
                      <div className="text-right shrink-0">
                        {rubricScore !== undefined ? (
                          <div className="text-xs font-bold text-emerald-800 bg-emerald-100 px-3 py-1 rounded-xl border border-emerald-300">
                            ได้: {rubricScore.score} / {rubric.maxScore} คะแนน
                          </div>
                        ) : (
                          <div className="text-xs font-semibold text-[#5A4D41] bg-white px-2.5 py-1 rounded-xl border border-[#D9CABB]">
                            คะแนนเต็ม: <strong className="text-[#B94E48]">{rubric.maxScore}</strong>
                          </div>
                        )}
                      </div>
                    </div>
        ...
        {/* RIGHT COLUMN: STICKY SUBMISSION BOX (col-span-5 / 4) */}
        <div
          id="submission-panel"
