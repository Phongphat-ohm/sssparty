...
import { ShieldCheck, Lock, User, Sparkles, ArrowLeft, Loader2 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#FFF9F0]">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl border border-[#EADBCC] p-8 space-y-6 relative overflow-hidden">
        ...
        <div className="absolute -top-12 -right-12 w-32 h-32 bg-[#D9A441]/15 rounded-full blur-2xl" />
        <div className="absolute -bottom-12 -left-12 w-32 h-32 bg-[#B94E48]/10 rounded-full blur-2xl" />
        ...
        <div className="text-center space-y-2 relative">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[#D9A441]/20 text-[#D9A441] mb-2 shadow-inner">
            <ShieldCheck className="w-8 h-8 text-[#B94E48]" />
          </div>
        ...
        <form action={formAction} className="space-y-4 relative">
          ...
          <button
            type="submit"
            disabled={isPending}
            className="w-full py-3.5 px-4 rounded-xl font-semibold text-white bg-[#B94E48] hover:bg-[#A33F39] active:scale-[0.99] transition-all shadow-md hover:shadow-lg disabled:opacity-60 flex items-center justify-center gap-2 cursor-pointer"
          >
            {isPending ? (
        ...
        <div className="pt-4 border-t border-[#EADBCC] text-center space-y-3">
          <Link
            href="/student-login"
            className="inline-flex items-center gap-1.5 text-sm font-medium text-[#C96B4B] hover:text-[#B94E48] transition-colors"
          >
            <ArrowLeft className="w-4 h-4" />
            เข้าสู่ระบบสำหรับนักเรียนส่งงาน
          </Link>
          <p className="text-xs text-[#A8988B]">
            ระบบห้องเรียนส่งงานและตรวจงานชุมนุมสื่อสร้างสรรค์
          </p>
        </div>
      </div>
    </div>
